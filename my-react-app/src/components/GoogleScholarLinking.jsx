...
//   }, [navigate]);
//
//   // Function to go back to research page
//   const handleGoBack = () => {
//     navigate('/research');
//   };
//
//   // Handle unlinking Google Scholar account for testing
...
const GoogleScholarLinking = ({ onLogout }) => {
  ...
  const fetchPublications = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8080/api/google-scholar/import', {
        method: 'POST',
        headers: {
      ...
      setScholarPublications(data.notInSystem);
      setSystemPublications(data.inSystem);
    } catch (error) {
  ...
  const handleAddPublication = async (e) => {
    ...
    // Send new publication to backend
    try {
      ...
      console.log('Publication added:', result);

      // Update local state
      setScholarPublications(scholarPublications.filter(pub => pub.id !== selectedPublication.id));
      setSystemPublications([...systemPublications, newPublication]);
  ...
  return (
      <div className="scholar-linking-root">
        ...
        <main className="scholar-linking-main">
          ...
          <div className="scholar-publications-section">
            <h3>Your Google Scholar Publications</h3>
            {scholarPublications.length > 0 ? (
                <div>
                  {scholarPublications.map(publication => (
                      <div key={publication.id} onClick={() => setSelectedPublication(publication)}>
                        <h4>{publication.title}</h4>
                        <p>{publication.authors}</p>
                        <button onClick={() => handleAddPublication(publication)}>
                          Add to System
                        </button>
                      </div>
                  ))}
                ...
                <div className="modal">
                  <form onSubmit={handleAddPublication}>
                    <input
                        type="text"
                        value={formData.citedAs}
                        onChange={(e) => setFormData({ ...formData, citedAs: e.target.value })}
                        placeholder="Cited As"
                    />
                    <input
                        type="text"
                        value={formData.doi}
                    ...
                    <input
                        type="file"
                        onChange={(e) => setFormData({ ...formData, supportingDocument: e.target.files[0] })}
                    />
                    <select
                        value={formData.sdg}
