...
const MultipleAddForm = ({ onLogout }) => {
  const navigate = useNavigate();
  const [activityType, setActivityType] = useState('publication');
  const [uploadedFile, setUploadedFile] = useState(null);
  ...
  const activityOptions = [
    ...
    { id: 'service', label: 'Public Services' },
    { id: 'teaching', label: 'Teaching Activities' }
  ];

  const handleDownloadCSV = () => {
    alert(`Download CSV template for ${activityType}`);
    // In a real app: Trigger download of the correct CSV template based on activityType
  };
  ...
  const handleFileUpload = async (e) => {
    ...
    const username = localStorage.getItem('username'); // Adjust this as needed
    ...
    switch (activityType) {
      ...
      default:
        ...
        setMessage('Error: Invalid activity type selected');
        setLoading(false);
        return;
    }

    const fullUrl = `http://localhost:8080/api/upload/${endpoint}`;

    console.log("Uploading CSV with headers:");
      ...
      <div className="multiple-add-root">
        ...
        <main className="multiple-add-main">
          <div className="multiple-add-header">
            ...
            <h1>Upload Multiple Entries</h1>
          </div>

          <div className="multiple-add-form-container">
            <form className="multiple-add-form" onSubmit={(e) => e.preventDefault()}>
              <div className="top-section">
                <div className="instructions">
                  <h2>Bulk Upload</h2>
                  <p>Select the activity type and upload a CSV file to add multiple entries.</p>
                  <p className="info-text">
                    The CSV upload does not support adding SDG and Supporting Document fields. You'll need to edit entries individually afterward.
                  </p>

                  <label htmlFor="activity-type">Select Activity Type:</label>
                  <select
                      id="activity-type"
                      value={activityType}
                ...
                <div className="action-buttons">
                  <button type="button" className="download-csv-btn" onClick={handleDownloadCSV}>
                    <FaDownload /> Download CSV Template
                  </button>
